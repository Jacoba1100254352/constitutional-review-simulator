...
def excluded_large_artifacts() -> list[Path]:
    return sorted((ROOT / "reports").glob("*-cases.csv.gz"))
...
- `replication-package.zip`: Java source, configuration, calibration source observations, generated aggregate reports, provenance manifests, and reproduction commands.
- `excluded-large-artifacts.txt`: raw case-level outputs omitted from the review ZIP to keep the upload practical; regenerate with the listed `make` commands.
...
def main() -> None:
    ...
    excluded_artifacts.write_text(
        "\n".join(
            [
                "# Large generated artifacts excluded from the anonymous review ZIP",
                "",

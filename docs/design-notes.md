...
promotion queue, and Markdown roadmap. The same generator writes
`config/court-profiles/profile-benchmark-cards.md`, which is the profile-level
review surface for checking a named court's validation rows, stress-only rows,
missing families, source-range misses, and promotion tasks together.

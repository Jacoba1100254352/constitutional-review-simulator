...
A dialogic-review vignette exposes the same problem in a different form. Suppose a high-mandate legislature enacts a rights-burdening criminal-procedure statute. A strong-form court can invalidate the statute immediately, concentrating final veto authority in the judiciary. A weak-form court can issue a declaration that preserves rights review while giving the legislature a response period. That is attractive only if the response cycle is credible. If the legislature can ignore the declaration, issue a symbolic reply, or bury the issue in committee, veto power has moved from a court order to legislative inaction. Rights-impact statements create a similar front-end risk: they can improve rights-sensitive lawmaking, but only when the screening process is transparent enough that it does not become an unreviewed gatekeeping veto.
...
The results separate direct implementation checks from downstream propagation effects. Direct checks include reason-giving, merits follow-up, mechanism activation, weak-form declarations, suspended declarations, and formal legislative-response triggers. They show whether the mechanism is operating as coded. Downstream effects include rights-claimant success, compliance, constitutional conflict, modeled public legitimacy, delay, institutional cost, and veto-relocation risk. Those downstream columns carry the substantive interpretation because they are less directly built into the mechanism labels.
...
Table~\ref{tab:weight-robustness} pushes that test further by sampling random normative weights over twelve baseline outputs. Architecture leadership is unstable across the sampled profiles, which supports the paper's emphasis on tradeoff maps rather than rank ordering. Weak-form review leads most sampled mechanism profiles, but that is not a design recommendation by itself. It is a modeling warning: weak-form review benefits from low final-veto risk and credible response assumptions, so future empirical work must stress legislative uptake, party-system fragmentation, and political-culture sensitivity before treating that dominance as robust.

\input{tables/weight_robustness_results}

Four patterns emerge from the experiments. First, emergency-process reforms produce the clearest direct movement on emergency irregularity, but their substantive value depends on downstream propagation: rights-claimant success, legitimacy, compliance, conflict, delay, and cost. Second, appointment, size, and selection reforms mostly act indirectly. They can change legitimacy and alignment through replacement dynamics, but they do not discipline emergency procedure as directly as written-reasons or merits-follow-through rules. Third, cross-checking and council designs tend to improve stability and lower reversal rates at the price of administrative load, upstream screening, and implementation complexity. Fourth, term limits and appointment systems matter most when linked to dynamic replacement over time; their effects are muted if composition is treated as fixed at initial court creation.

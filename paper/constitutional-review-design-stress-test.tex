...
Table~\ref{tab:validation-miss-interpretation} records the current miss status. All validation-counted source-range checks now fall within documented ranges, so the table reports a no-current-miss state rather than a list of unresolved benchmark failures. If future source rows fall outside range, intake-rate misses should usually be read as denominator problems, emergency-relief misses as procedure-specific assumptions about applicant mix and merits follow-through, and weak-form response misses as questions about the quality and timing of legislative response rather than court merits behavior.

\input{tables/validation_miss_interpretation}

\subsection{Remaining Source-Range Gaps}

The source-range checks are better at finding missing mechanisms than at confirming model fit. All validation-counted rows currently fall within their documented ranges, but this should be read as a calibration audit rather than country forecasting. Pipeline variables such as generated filing pools, leave filters, lower-court conflict formation, repeat-player selection, and public-interest support are still stylized. Compliance and enforcement measures are downstream model outputs, not source-fitted country estimates. Emergency-docket outputs distinguish reasons, vote disclosure, public disagreement, government applicants, merits follow-up, and relief, but they do not yet use a docket-level dataset of applications, responses, referral decisions, and subsequent merits disposition.

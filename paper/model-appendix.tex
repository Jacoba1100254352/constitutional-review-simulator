...
\midrule
Case facts & Bounded synthetic or imported values for rights threat, mandate, support, urgency, ambiguity, salience, lower-court conflict, and executive pressure. & Rights threat with salience and urgency; ambiguity with lower-court conflict; support inversely with rights threat. & Design-prior docket generator. \\
World context & Appointment polarization, emergency pressure, legislative conflict, public trust, partisan pressure, fragmentation, government control, electoral pressure, civil society, implementation, and legal-tradition compatibility. & Political conflict affects public trust, compliance, response, and appointment pressure. & Stress-case controls; country-context rows are diagnostic only. \\
Institutional rules & Scenario-coded appointment, size, tenure, removal, recusal, docket control, emergency procedure, review timing, threshold, council/cross-checking, override, and weak-form flags. & Rules interact with case facts through access, voting, emergency, response, compliance, and cost models. & Institutional mapping or synthetic mechanism. \\
...
External targets & Source-backed target ranges only when URL, denominator, construction note, and direct simulator analogue are present. & None assumed; misses are calibration roadmap items. & External diagnostic comparison. \\
\bottomrule
\end{tabular}
\end{center}
...
Campaign uncertainty bands use a cluster bootstrap over run-level outputs. They measure run-block simulation variation inside the specified model, not uncertainty about model specification, variable construction, weight choice, country mapping, or normative theory. External diagnostic comparisons compare simulated columns to source-backed calibration rows only when a row has a public URL, nonzero denominator, construction note, and direct simulator analogue. Context rows, public-trust proxies, normalized cost estimates, and unverified source candidates remain visible in the supplement but are not counted as empirical validation evidence.

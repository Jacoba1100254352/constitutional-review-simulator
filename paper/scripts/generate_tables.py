...
def generate_mechanism_results() -> None:
    ...
    lines = [
        ...
        r"\begin{table}[hbt!]",
        r"\centering",
        r"\caption{Constitutional-review mechanism results.}",
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\begin{minipage}{0.96\linewidth}",
            r"\footnotesize Notes: Rows are mechanism scenarios under the baseline generated docket. RIS, Pre, and Abs report rights-impact statement, pre-enactment review, and abstract review rates separately. Lower veto-relocation risk and political-culture sensitivity are better; higher democratic constitutionalism and transplant feasibility are better.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
...
def generate_validation_summary() -> None:
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\begin{minipage}{0.96\linewidth}",
            r"\footnotesize Notes: The source-check campaign runs stylized archetype presets against a shared benchmark docket. Counts include only source-backed target rows marked for validation in the calibration source matrix. Rows with synthesis source labels, missing denominators, contextual public-trust proxies, or normalized-cost construction are excluded and shown as not counted.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
...
def generate_validation_miss_interpretation() -> None:
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\begin{minipage}{0.96\linewidth}",
            r"\footnotesize Notes: The table reports the eight largest out-of-range source checks. The full generated miss report is retained in \texttt{reports/constitutional-review-validation-v1-misses.csv} and \texttt{.md}. Misses are priorities for future calibration.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]

...
def generate_baseline_results() -> None:
    ...
    lines = [
        ...
        r"\begin{table}[hbt!]",
        r"\centering",
        r"\caption{Court-architecture stress-test diagnostics.}",
...
def generate_mechanism_results() -> None:
    ...
    lines = [
        ...
        r"\scriptsize",
        r"\setlength{\tabcolsep}{2pt}",
        r"\begin{tabular}{@{}lL{0.20\linewidth}rrrrrrrrrr@{}}",
        r"\toprule",
        r"Code & Mechanism & Dem. const. & Veto $\downarrow$ & Transplant & Culture $\downarrow$ & Weak & Susp. & RIS & Pre & Abs & Response \\",
        r"\midrule",
    ]
        ...
        lines.append(
            " & ".join(
                [
                    ...
                    num(row["rightsImpactStatementRate"]),
                    num(row["preEnactmentReviewRate"]),
                    num(row["abstractReviewRate"]),
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\begin{minipage}{0.96\linewidth}",
            r"\footnotesize Notes: These are synthetic mechanism tests, not claims about named legal systems. RIS, Pre, and Abs report rights-impact statement, pre-enactment review, and abstract review rates separately so the front-end screening mechanisms are not collapsed into one maximum. Access effects remain in the generated reports through the synthetic case-selection access diagnostic, which combines litigant capacity, public-interest support, repeat-player advantage, and intake filtering.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
...
def generate_validation_summary() -> None:
    ...
    lines = [
        ...
        r"\begin{table}[hbt!]",
        r"\centering",
        r"\caption{External diagnostic fit checks against documented target ranges.}",
    ...
    lines.extend(
        [
            ...
            r"\end{tabular}",
            r"\begin{minipage}{0.96\linewidth}",
            r"\footnotesize Notes: The diagnostic campaign runs stylized archetype presets against a shared benchmark docket. The checks count only source-backed target rows marked for validation in the calibration source matrix. Rows with synthesis source labels, missing denominators, contextual public-trust proxies, or normalized-cost construction are excluded and shown as not counted. These are external diagnostic comparisons, not fitted validation estimates.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
...
def generate_validation_miss_interpretation() -> None:
    ...
    lines = [
        ...
        r"\begin{table}[hbt!]",
        r"\centering",
        r"\caption{Calibration roadmap from the largest source-specific diagnostic misses.}",

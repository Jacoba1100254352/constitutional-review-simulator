...
VALIDATION_CALIBRATION_CSV = REPORT_DIR / "constitutional-review-validation-v1-calibration.csv"
...
CALIBRATION_ROWS = [
    ...
    ("canada-charter-dialogue-1982-2007.csv", "canada-charter-dialogue-1982-2007", "statute_nullification_rate"),
...
VALIDATION_PROFILES = [
    ...
    ("canada-scc-recent", "canadian-supreme-court", "Canada SCC multi-year intake"),
    ("canada-charter-dialogue-1982-2007", "canadian-supreme-court", "Canada Charter historical"),
...
def generate_reference_items() -> None:
    ...
    for entry in entries:
        ...
        if entry["entryType"] == "book":
            ...
            doi = rf" \url{{https://doi.org/{entry['doi']}}}." if entry.get("doi") else ""
            lines.append(rf"{author_text} {cited_year}. ``{title}.'' \textit{{{journal}}} {volume}{number}{pages}.{doi}")
        else:
            url_match = re.search(r"\\url\{([^}]+)\}", entry.get("howpublished", ""))
            url = url_match.group(1) if url_match else entry.get("howpublished", entry.get("url", ""))
            note = entry.get("note", "")
...
def generate_validation_summary() -> None:
    ...
    # Preserve every period-specific check and the runner's unrounded pass flag.
    # A profile/metric lookup collapses repeated historical periods and can omit
    # new metrics or disagree with comparisons near a rounded range boundary.
    for row in read_rows(VALIDATION_CALIBRATION_CSV):

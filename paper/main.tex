...
The narrowness of the table is itself a result. Comparative rows are excluded from the validation count when they rely on synthesis labels, missing denominators, public-trust proxies, unsupported target keys, or normalized-cost assumptions. Only France QPC and CJEU route-mix rows with source-specific denominators and direct simulator columns are counted. That leaves a smaller validation-style surface and makes the limitation explicit: the framework is ready for comparative design stress testing, not for estimating the behavior of named courts. Empirical extensions should replace each contextual row with a source-specific dataset before using the word validation for country profiles.

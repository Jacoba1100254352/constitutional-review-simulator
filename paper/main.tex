...
The next research increment should add court-specific calibration packs, richer case-flow validation against specific courts and periods, and source-backed evidence for legislative response, compliance, enforcement, transplant feasibility, political culture, and case-selection access. The repository now separates those tasks into research-roadmap CSV files rather than treating starter assumptions as calibration targets. Future empirical work should replace synthesized targets with denominator-backed datasets, model political-culture variables with external indicators rather than internal assumptions, add explicit implementation budgets for access institutions, and test whether legislative response cycles are credible under different party-system and compliance conditions. A Journal of Law and Courts submission should keep the main text compact and move large scenario tables, source matrices, raw case records, and sensitivity reports to supplementary materials.

\section*{Supplementary Material}
The supplementary replication package is designed to include source code and tests in \path{src/main/java} and \path{src/test/java}; calibration, research-roadmap, and benchmark inputs in \path{config/calibration}, \path{config/research}, \path{config/comparative}, \path{config/pipeline}, \path{config/emergency}, and \path{config/cost-benchmarks}; the normalized calibration source-observation matrix; compact generated reports; figure- and table-generation scripts; and the Makefile commands listed in the repository README. Full compressed case-level and interval exports are reproducible from those commands and should be included in the final public deposit or supplied separately if requested. For anonymous review, any repository metadata or file paths that identify the author should be removed or redacted from the submitted archive.

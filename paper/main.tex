...
\[
R = A_s+.26T_r+.22S_c+.16L_c+.22C_p+.12I_c+.18E+.16O+.13D+.10P+.12B-.05U,
\]
where $A_s$ is structural access, $T_r$ rights threat, $S_c$ constitutional salience, $L_c$ lower-court conflict, $C_p$ certiorari or intake pressure, $I_c$ intercourt conflict, $E$ emergency status, $O$ ombudsman trigger, $D$ public-defender participation, $P$ pre-enactment review, $B$ abstract review, and $U$ legal ambiguity. The case is reviewed when $R$ multiplied by jurisdictional access plus random docket noise exceeds the review threshold.
...
Democratic constitutionalism & democratic constitutionalism, veto-relocation risk, transplant feasibility, political-culture sensitivity & mixed \\
...
\[
S=\frac{1}{29}\left(\sum_{m\in H}m+\sum_{m\in L}(1-m)\right).
\]
The higher-is-better set includes legal stability, rights protection, legitimacy, democratic responsiveness, democratic constitutionalism, independence/accountability balance, compliance, executive implementation, local-government compliance, emergency reason-giving, emergency vote disclosure, legal-transplant feasibility, legislative response, and rights-impact statements. The lower-is-better set includes partisan alignment, shadow-docket abuse, reversal rate, emergency relief rate, merits invalidation rate, constitutional conflict, veto-relocation risk, political-culture sensitivity, defiance, workarounds, agency nonacquiescence, legislative reenactment, emergency public disagreement, administrative load, and total institutional cost. Diagnostic metrics such as review rate, recusal rate, override rate, weak-form declaration rate, abstract-review rate, lower-court depth, replacement pressure, and public trust remain visible in reports but do not enter $S$.
...
\caption{Baseline institutional cost versus democratic constitutionalism. The vertical axis is a synthetic diagnostic combining rights, responsiveness, legitimacy, compliance, conflict, veto-relocation risk, feasibility, and culture sensitivity. CUR* marks the current-style scenario.}
...
The simulator is intentionally abstract. It compresses doctrinal variation, legal culture, party systems, federal design, and executive implementation into variables that can be compared across institutional scenarios. It does not yet model litigant strategy, judicial opinion assignment, actual precedent networks, budget appropriations, media framing, or reform backlash as independently strategic actors. Legal-transplant feasibility and political-culture sensitivity are diagnostic constructs, not measured country-level estimates. Implementation cost is normalized to the simulator scale and should not be read as a budget forecast. The source-specific validation surface is narrow; most comparative rows remain stress-test assumptions until paired with reproducible empirical datasets.

The next research increment should add richer case-flow validation against specific courts and periods, distinguish litigant selection from judicial selection more explicitly, and replace synthesized calibration targets with source-specific datasets. It should also model political-culture variables with external indicators rather than internal assumptions, add explicit implementation budgets for access institutions, and test whether legislative response cycles are credible under different party-system and compliance conditions. A Journal of Law and Courts submission should keep the main text compact and move large scenario tables, source matrices, raw case records, and sensitivity reports to supplementary materials.

\section*{Supplementary Material}
The supplementary replication package is designed to include source code and tests in \path{src/main/java} and \path{src/test/java}; calibration and benchmark inputs in \path{config/calibration}, \path{config/comparative}, \path{config/pipeline}, \path{config/emergency}, and \path{config/cost-benchmarks}; the normalized calibration source-observation matrix; compact generated reports; figure- and table-generation scripts; and reproducibility commands including \texttt{make test}, \texttt{make campaign}, \texttt{make paired-campaign}, \texttt{make validation-check}, \texttt{make sensitivity-check}, \texttt{make paper}, and \texttt{make supplement}. Full compressed case-level and interval exports are reproducible from those commands and should be included in the final public deposit or supplied separately if requested. For anonymous review, any repository metadata or file paths that identify the author should be removed or redacted from the submitted archive.

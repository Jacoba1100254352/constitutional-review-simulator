...
`reports/*-cases.csv.gz`, `reports/*-objects.csv.gz`, and interval CSVs to keep
the upload practical; those outputs are regenerated by the campaign commands and listed in
...
## Case and Object Exports and Uncertainty Bands
...
most calibration intervals remain conservative denominator-based checks where there
is no direct case-level analogue. Statute-nullification calibration intervals
use run-block resampling. The compressed case-level and interval files
...
Each campaign also writes `*-objects.csv.gz`, with challenged statutes,
regulations, and government conduct kept distinct. Statute nullifications and
statute dispositions are pooled separately from case-level invalidations.
Object generation and conditional relief allocation are explicitly synthetic,
not estimated historical docket or voting models. The standard campaign
commands independently reconcile raw object counts against case and aggregate
exports. Object files are excluded from the compact anonymous review ZIP and
can be regenerated for a full public deposit.

See [the measurement contract](docs/object-measurement-contract.md),
[the source evidence audit](docs/measurement-evidence-audit.md), and the generated
`reports/constitutional-review-measurement-audit-v1.csv`. The audit adds the
Canadian 19/50 statute-disposition check and eight denominator-backed German
and South African observations. The latter remain outside validation because
their procedural units, censoring, or source-coverage limitations do not match
